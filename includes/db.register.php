	 ...
	 if(isset($_POST['register_btn'])) {
		 ...
		 try{
			 ...
			 if(($password === $passwordRepeat && $password != null) && ($username != null && preg_match('/^[a-zA-Z0-9]{5,}$/', $username)) && 
				 ...
				 $sqlQuery = "INSERT INTO users(name, surname, username, email, password) VALUES('{$name}', '{$surname}', '{$username}', '{$email}', '{$password}')";
				 $mysqliQueryCon = $db_connection->query($sqlQuery);
